...
export function TaskBoard() {
  ...
  const fetchTasks = useCallback(async () => {
    setLoading(true);
    try {
      const data = await apiClient.getTasks(true);
      const taskList = data.tasks || [];
      setTasks(taskList);
      const notionProviders = [
        ...new Set(taskList.filter((t: Task) => t.providerType === "notion").map((t: Task) => t.providerId)),
      ];
      for (const pid of notionProviders) {
        try {
          const res = await apiClient.getNotionStatusOptions(String(pid));
          setNotionStatusOptions((prev) => ({ ...prev, [String(pid)]: res.options || [] }));
        } catch {
          // ignore
        }
      }
  ...
  const columns: { id: ColumnId; label: string; icon: typeof Circle; color: string }[] = [
    { id: "todo", label: COLUMN_LABELS.todo, icon: Circle, color: "text-blue-500 dark:text-blue-400" },
    { id: "in_progress", label: COLUMN_LABELS.in_progress, icon: Clock, color: "text-amber-500 dark:text-amber-400" },
    { id: "done", label: COLUMN_LABELS.done, icon: CheckCircle2, color: "text-emerald-500 dark:text-emerald-400" },
    { id: "blocked", label: COLUMN_LABELS.blocked, icon: AlertCircle, color: "text-rose-500 dark:text-rose-400" },
  ];
  ...
  const handleUpdateStatus = async (taskId: string, newStatus: string) => {
    ...
    try {
      await apiClient.updateTask({ id: taskId, status: statusStr });
      fetchTasks();
    } catch {
      fetchTasks();
    }
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    const options =
      task?.providerType === "notion" && task?.providerId ? notionStatusOptions[task.providerId] : null;
  ...
  async function handleToggleComplete(task: Task) {
    ...
    const newStatus = options?.length
      ...
      : isCompleted
        ...
        : "Done";
    try {
      await apiClient.updateTask({ id: task.id, completed: !isCompleted, status: newStatus });
      fetchTasks();
    } catch (error) {
  ...
  async function handleCreateTask() {
    ...
    try {
      await apiClient.createTask({ action: "create", title: newTaskTitle.trim() });
      setIsCreating(false);
      setNewTaskTitle("");
      fetchTasks();
    } catch (error) {
  ...
  async function handleDeleteTask(taskId: string) {
    ...
    try {
      await apiClient.deleteTask(taskId, "delete");
      fetchTasks();
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      {/* Toolbar */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 border-b border-border bg-card sticky top-0 z-20">
        <div className="flex items-center gap-4 w-full sm:w-auto">
          <div className="flex items-center gap-2">
            <div className="p-2 bg-accent/10 rounded-lg">
              <CheckSquare className="h-5 w-5 text-accent" />
            </div>
            <h1 className="text-lg font-semibold text-foreground">Takenbord</h1>
          </div>
          <div className="flex items-center bg-muted/50 rounded-lg p-1 border border-border">
            <button
              onClick={() => setViewMode("board")}
              className={`p-1.5 rounded-md transition-all ${viewMode === "board" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground"}`}
            >
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          <div className="relative flex-1 sm:w-64">
            ...
            <input
              ...
              className="w-full bg-muted/50 border border-border rounded-lg pl-9 pr-4 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-accent"
            />
          </div>
          <button
            onClick={() => setIsCreating(true)}
            className="flex items-center gap-2 bg-accent text-accent-foreground px-4 py-2 rounded-lg text-sm font-medium hover:bg-accent-hover transition-colors"
          >
                  ...
                  <Droppable droppableId={column.id}>
                    {(provided, snapshot) => (
                      ...
                      >
                        ...
                        {filteredTasks.filter((t) => getTaskStatus(t) === column.id).length === 0 &&
      ...
      {isCreating && (
        ...
        >
          ...
          >
            <div className="p-6">
              <h2 className="text-lg font-semibold text-foreground mb-4">Taak toevoegen</h2>
              <input
                type="text"
                autoFocus
                ...
                className="w-full bg-muted/50 border border-border rounded-xl px-4 py-3 text-base text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent/50"
              />
              <p className="mt-2 text-xs text-muted-foreground">Handmatige taken worden lokaal opgeslagen.</p>
            </div>
            <div className="flex items-center justify-end gap-2 p-4 bg-muted/30 border-t border-border">
              ...
              <button
                ...
                className="bg-accent text-accent-foreground px-6 py-2 rounded-lg text-sm font-medium hover:bg-accent-hover disabled:opacity-50"
              >
...
function TaskDetailModal({
  ...
  async function handleSave() {
    ...
    setIsSaving(true);
    try {
      await apiClient.updateTask({ id: task.id, title, description });
      onUpdate();
      onClose();
    } catch (error) {
  ...
  async function handleOpenExternal() {
    if (task.externalUrl) {
      try {
        await openUrl(task.externalUrl);
      } catch (e) {
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <div
              className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider border ${
                task.providerType === "notion"
                  ? "bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-zinc-200 border-zinc-200 dark:border-zinc-700"
                  : task.providerType === "github"
                    ? "bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-200 border-slate-200 dark:border-slate-700"
            ...
            <div className="flex items-center gap-1">
              <button
                onClick={onDelete}
                className="p-1.5 text-muted-foreground hover:text-red-500 dark:hover:text-red-400 hover:bg-red-500/10 rounded-md"
              >
            ...
            <div>
              ...
              {isManual ? (
                <input
                  ...
                  className="w-full bg-muted/50 border border-border rounded-lg px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-accent/50"
                />
              ) : (
            ...
            <div>
              ...
              {isManual ? (
                <textarea
                  ...
                  className="w-full bg-muted/50 border border-border rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent/50 resize-none"
                />
              ) : (
                <div className="text-sm text-muted-foreground whitespace-pre-wrap max-h-48 overflow-y-auto pr-2">
            ...
            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-border">
              <div>
                ...
                <div className="text-xs text-foreground flex items-center gap-2">
                  <div
                    className={`w-2 h-2 rounded-full ${task.completedAt ? "bg-emerald-500" : "bg-blue-500"}`}
                  />
                  {task.status || (task.completedAt ? "Voltooid" : "Te doen")}
                </div>
              </div>
              <div>
        ...
        <div className="flex items-center justify-end gap-2 p-4 bg-muted/30 border-t border-border">
          {task.externalUrl && (
            ...
            >
              ...
              Openen in {task.providerType === "notion" ? "Notion" : task.providerType === "github" ? "GitHub" : "bron"}
            </button>
          )}
          <button
            onClick={onClose}
...
function TaskCard({
  ...
  const statusOptions = (hasCurrent ? opts : task.status ? [task.status, ...opts] : opts).map((v) => ({
    value: v,
    label: v,
  }));
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div
            className={`text-sm font-medium leading-tight mb-1 ${isCompleted ? "line-through text-muted-foreground" : "text-foreground"}`}
          >
            {task.title}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 mt-2">
            <div
              className={`flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-bold uppercase tracking-tighter border ${
                task.providerType === "notion"
                  ? "bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-zinc-200 border-zinc-200 dark:border-zinc-700"
                  : task.providerType === "github"
                    ? "bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-slate-200 border-slate-200 dark:border-slate-700"
            ...
            >
              {task.providerType === "notion" ? "Notion" : task.providerType === "github" ? "GitHub" : "Lokaal"}
            </div>
            ...
            {task.scheduledAt && (
              <div className="flex items-center gap-1 text-[10px] text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/50 px-1.5 py-0.5 rounded font-medium border border-emerald-200 dark:border-emerald-800">
      ...
      <div className="absolute top-2 right-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <button
          ...
          className="p-1 text-muted-foreground hover:text-red-500 dark:hover:text-red-400 hover:bg-red-500/10 rounded"
        >
      ...
      {task.providerType === "notion" && (
        <div
          className="mt-3 pt-2 border-t border-border/50 flex items-center justify-between"
          onClick={(e) => e.stopPropagation()}
        >
          <span className="text-[9px] text-muted-foreground font-medium uppercase tracking-tight">Status</span>
          <select
            value={task.status || "Todo"}
            onChange={(e) => onUpdateStatus(e.target.value)}
            className="text-[10px] bg-muted/80 border border-border rounded px-1.5 py-0.5 focus:ring-0 cursor-pointer font-medium text-foreground"
          >
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="p-2 text-muted-foreground hover:text-red-500 dark:hover:text-red-400 rounded-lg"
          >
            <Trash2 className="h-4 w-4" />
          </button>
