    ...
    <div className="space-y-6">
      ...
      {activeTab === "general" ? (
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          <SettingSection title="Tekst uitlijning" description="Uitlijning van de tekst in events">
            <div className="flex gap-2">
              {([
                { value: "left" as const, label: "Links" },
                { value: "center" as const, label: "Midden" },
                { value: "right" as const, label: "Rechts" },
              ]).map((opt) => (
                <OptionButton
                  key={opt.value}
                  label={opt.label}
                  selected={settings.eventTextAlignment === opt.value}
                  onClick={() => updateSettings({ eventTextAlignment: opt.value })}
                />
              ))}
            </div>
          </SettingSection>
