...
export type WeekViewZoom = 3 | 5 | 7;
...
interface MobileWeekViewProps {
  ...
  onWeekViewZoomChange: (zoom: WeekViewZoom) => void;
}
...
export const MobileWeekView = memo(function MobileWeekView({
  ...
  const weekDays = useMemo(
    () => getDaysAround(currentDate, weekViewZoom, settings.weekStartsOn),
    [currentDate, weekViewZoom, settings.weekStartsOn]
  );
  ...
  const zoomOptions: { value: WeekViewZoom; label: string }[] = [
  ...
  return (
    <div className="flex h-full flex-col safe-left safe-right">
      {/* Zoom toggle - 3/5/7 dagen */}
      <div className="mb-2 flex shrink-0 gap-1 rounded-lg border border-border bg-muted/30 p-1">
        {zoomOptions.map((opt) => (
          <button
            key={opt.value}
            onClick={() => onWeekViewZoomChange(opt.value)}
            className={`flex min-h-[36px] flex-1 items-center justify-center rounded-md px-1 py-1.5 text-[10px] font-medium transition-colors ${
              weekViewZoom === opt.value
      ...
      <div className="flex flex-1 flex-col min-h-0">
        <div className="shrink-0 border-b border-border">
          <div className="flex">
            <div className="flex w-[36px] shrink-0 items-end justify-center pb-1">
            ...
            {weekDays.map((day) => {
              ...
              return (
                <div
                  key={dayKey}
                  className="flex flex-1 flex-col items-center border-l border-border-light py-1.5"
                >
                  <span
                    className={`text-[10px] font-medium uppercase tracking-wider ${
                      mounted && today ? "text-foreground" : "text-muted-foreground"
                    }`}
                  >
                    {formatWeekDay(day)}
                  </span>
                  <span
                    className={`mt-0.5 flex h-7 w-7 items-center justify-center rounded-md font-pixel text-sm ${
                  ...
                  {multiDaySpans.map((span, idx) => (
                    <div
                      ...
                      style={{
                        position: "absolute",
                        top: `${idx * 22}px`,
                        left: `${(span.startCol / columnCount) * 100}%`,
                        width: `${(span.span / columnCount) * 100}%`,
                        height: "20px",
                        padding: "0 2px",
                      }}
                    >
                      ...
                      >
                        <div
                          ...
                          style={{ backgroundColor: span.event.color || "#737373" }}
                        />
                    ...
                    return (
                      ...
                      >
                        ...
                        {dayTodos.map((todo) => (
                          <div
                            key={todo.id}
                            className="flex w-full items-center gap-1 truncate rounded-[4px] bg-muted px-1.5 py-0.5 text-[10px] text-muted-foreground"
                          >
              ...
              <div
                style={{
                  height:
                    ...
                    (Array.from(singleDayAllDay.values()).some((v) => v.length > 0) ? 22 : 0) +
                    2,
                }}
              />
            </div>
