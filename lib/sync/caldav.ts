...
/**
 * Create a CalDAV client for a generic CalDAV server.
 */
...
async function getClientForAccount(
  ...
  // For CalDAV, accessToken stores the password (encrypted)
  // providerData stores { serverUrl, username }
  const providerData = account.providerData as Record<string, unknown>;
  const serverUrl = providerData?.serverUrl as string;
  const username = providerData?.username as string;

  if (!serverUrl || !username) {
    throw new Error("CalDAV server URL or username not found");
  }
...
/**
 * Fetch all calendars from a CalDAV account and sync to DB.
 */
export async function syncCalDAVCalendars(accountId: string) {
...
/** Sync events from a CalDAV calendar (uses shared sync logic). */
export async function syncCalDAVEvents(accountId: string, calendarId: string) {
  const { client } = await getClientForAccount(accountId);
  const { syncCalDAVEventsFromClient } = await import("./caldav-sync");
  await syncCalDAVEventsFromClient(client, accountId, calendarId, "CalDAV");
}
...
export async function updateCalDAVEvent(
  ...
  if (!event.externalId || !event.icsData) {
    throw new Error("Event not synced to CalDAV or missing ICS data");
  }
...
export async function deleteCalDAVEvent(
  ...
  // If no externalId, event was never synced to CalDAV, nothing to delete
  if (!event.externalId) {
    return;
  }
